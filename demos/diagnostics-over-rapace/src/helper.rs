...
async fn accept_inherited_stream() -> Option<TcpStream> {
    ...
    eprintln!(
        "[diagnostics-plugin] Accepted inherited stream from {:?}",
        peer
    );
    Some(stream)
}
    ...
    match args.transport {
        TransportType::Stream => {
            if let Some(stream) = accept_inherited_stream().await {
                eprintln!("[diagnostics-plugin] Using inherited TCP listener");
            ...
            if addr.contains(':') {
                // TCP
                eprintln!("[diagnostics-plugin] Connecting to TCP address: {}", addr);
                ...
                #[cfg(unix)]
                {
                    ...
                    eprintln!("[diagnostics-plugin] Connecting to Unix socket: {}", addr);
